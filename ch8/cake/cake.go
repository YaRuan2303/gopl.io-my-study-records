...
func (s *Shop) baker(baked chan<- cake) { //发送 ，烘烤baked通道
	for i := 0; i < s.Cakes; i++ {
		c := cake(i) //类型转换
		if s.Verbose {
			fmt.Println("baking", c)
		}
		work(s.BakeTime, s.BakeStdDev) //延时
		baked <- c
	}
	close(baked)
}
...
func (s *Shop) icer(iced chan<- cake, baked <-chan cake) {
	for c := range baked {
		...
		work(s.IceTime, s.IceStdDev) //延时，制作ice时间
		iced <- c
	}
}
...
func (s *Shop) inscriber(iced <-chan cake) {
	for i := 0; i < s.Cakes; i++ {
		c := <-iced
		if s.Verbose {
			fmt.Println("inscribing", c) //这是什么打印？
		}
